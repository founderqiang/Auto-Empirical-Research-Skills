...
5. **写状态文件** `00_meta/workflow_state.json`（模板见
   [`assets/workflow_state.template.json`](assets/workflow_state.template.json)）。记录 `entry_stage`、
   `mode`、`target_journal`、`language`、`started=NOW`，以及每个 Stage 的
   `pending|in_progress|done|skipped`。**每个阶段开始置 `in_progress`、完成置 `done`**——这就是
   断点续跑的依据。
