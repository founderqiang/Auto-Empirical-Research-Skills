...
Both candidates below took the identical exam: the **16 deterministic benchmark tasks** in [`benchmark/tasks/`](../benchmark/tasks/), graded by the same checker that recomputes every data-derived gold from the committed CSVs (fabricated numbers fail the honest-* golds).

- **reference** — the committed stdlib pipelines: **16/16 tasks with every required gold passing**.
- **naive baseline** — a synthetic agent with no rigor layer (the folk move on each task, constructed deterministically from the reference numbers): fails required golds on **16/16 tasks**.
