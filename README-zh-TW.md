...
| 每次執行從資料**重算 gold 值**的數值基準任務 | **13** | [`benchmark/`](benchmark/) |
| 行為級**評測場景 / rubric 條目** | **26 / 138** | [`eval-harness/`](eval-harness/) |
...
| 層 | 它能擋住什麼 | 在哪 |
|---|---|---|
| **數值基準** | 報告數字與真實資料重算真值不符 —— 樸素 DID 符號陷阱、缺第一階段 F 的弱 IV、交錯時點下的 TWFE 偏誤、RDD 趨勢混淆、後處理壞控制、被合併均值掩蓋的異質效應（CATE）、只看均值漏掉尾部的分位數效應 | [`benchmark/`](benchmark/) · 13 任務 |
| **評測套件** | 散文級失誤：弱 IV 假性安心、交錯 DID 誤用 TWFE、編造引用、不安全的 `curl \| bash` 安裝、多重檢驗濫用、AER 合規缺口 | [`eval-harness/`](eval-harness/) · 26 場景 / 138 rubric |
